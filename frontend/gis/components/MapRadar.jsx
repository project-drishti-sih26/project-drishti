...
const MapRadar = ({
  ...
  useEffect(() => {
    ...
    locations.forEach((loc, index) => {
      ...
      // `|| 0.8` invented an 80% risk score for any target whose score did not
      // resolve. Show the tier instead of a number we do not have.
      const hasScore = typeof loc.riskScore === 'number' && loc.hasRealScore !== false;
      const scorePercent = hasScore ? Math.round(loc.riskScore * 100) : null;
      const tier = String(loc.riskTier || '').toUpperCase();
      const scoreColor = tier === 'CRITICAL' ? 'text-red-400'
    ...
    //
    // `riskCells || locations` looks like a fallback but never was one: an empty
    // array is truthy in JavaScript. The ML engine sends no alert-level
    // `risk_cells` (each ranked target carries its own `h3_index`), so
    // riskCells normalised to [] and every live alert rendered ZERO hexagons —
    // silently dropping the H3 spatial layer, the map's headline feature. Only
    // a non-empty riskCells array should win.
    const hexSource = (Array.isArray(riskCells) && riskCells.length > 0)
      ? riskCells
      : locations;
    const renderHexes = () => {
      try {
        addH3HexLayers(map, hexSource);
      } catch (err) {
